...
public class NoticeJooqRepository extends AutoAuditJooqRepository<NoticeRecord> implements NoticeRepository {
	...
	public Optional<Notice> findById(Long id) {
		...
		return dsl.select(NOTICE.asterisk(),
						  createUser.NAME.as("created_user_name"),
						  modifyUser.NAME.as("modified_user_name")
				  )
				  .from(NOTICE)
				  .leftJoin(createUser)
				  .on(NOTICE.CREATED_BY.eq(createUser.UNIQUE_ID))
				  .and(createUser.STATUS.eq(UserStatus.ACTIVE))
				  .leftJoin(modifyUser)
				  .on(NOTICE.MODIFIED_BY.eq(modifyUser.UNIQUE_ID))
				  .and(modifyUser.STATUS.eq(UserStatus.ACTIVE))
				  .where(NOTICE.ID.eq(id))
				  .fetchOptional(record -> Notice.builder()
											   .id(record.get(NOTICE.ID))
											   .revision(record.get(NOTICE.REVISION))
											   .viewCount(record.get(NOTICE.VIEW_COUNT))
											   .status(record.get(NOTICE.STATUS))
											   ...
											   .metadata(Metadata.builder()
															 .createdBy(record.get(NOTICE.CREATED_BY))
															 .createdByName(record.get("created_user_name", String.class))
															 .createdAt(record.get(NOTICE.CREATED_AT))
															 .modifiedBy(record.get(NOTICE.MODIFIED_BY))
